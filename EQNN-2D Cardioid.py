...
def parametricSolutions(t, w, nn, x1):
    ...
    N1 = nn(t,w)[0]
    f = C*(t**2 + w*(w+q) - torch.sqrt(t**2 + w**2))
    psi  = x1  + f*N1
    return psi
...
def region(X,Y):
    ...
    while j != n_train-1:
        ...
        if (x_0**2 + y_0*(y_0+q))/np.sqrt(x_0**2+y_0**2) <= 1:
            New_x.append(x_0)
            New_y.append(y_0)
            j += 1
        else:
...
for i in range(len(tTest)):
    for j in range(len(wTest)):
        ...
        if tTest[i]**2 + wTest[j]*(wTest[j]+q) - torch.sqrt(tTest[i]**2 + wTest[j]**2) > 0:
            psi[j][i] = 0
...
plt.ylabel('Loss')
plt.title('Loss History for Cardioid')

fig3 = plt.figure(figsize=(9,9))
plt.plot(t,Ehistory,'r')
plt.xlabel('Timesteps')
plt.ylabel('Energy')
plt.title('Energy History for Cardioid')
#plt.xscale('log')
plt.show()
